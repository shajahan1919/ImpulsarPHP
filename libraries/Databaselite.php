<?php
	/*
		...
		Description	: To work with SQL Server
		Author		: Shajahan Basha Syed

	*/

	include_once CONFIG_LIBRARIES.'/table.php';
	...
	class Databaselite{
		...
		public function connection(){
			$status = mysqli_connect($this->dbhost,$this->dbuser,$this ->dbpass);
			return $status;
		}
		...
		public function opendb(){
			$status = mysqli_select_db($this->conn,$this->dbname);
			return $status;
		}
		...
		public function fetch($querystatus){
			$resultset = mysqli_fetch_array($querystatus,MYSQLI_ASSOC);
			return $resultset;
		}

		public function countData($querystatus){
			return mysqli_num_rows($querystatus);
		}
		...
		public function fieldDetails($querystatus,$i=0){
			$field = array();
		...
		public function fieldName($querystatus,$i=0){
			return mysql_field_name($querystatus, $i);
		}

		public function escapeString($unescapedString){
			return mysqli_real_escape_string($this->conn,$unescapedString);
		}
